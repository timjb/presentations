...
\begin{frame}[t,fragile]
  ...
  \begin{onlyenv}<1-7>
    \begin{tabular}{r l}
      ...
      \visible<6->{Funktion \hask{s -> [a]}} \\

      \visible<5->{\alert<5>{Getter}} &
      \visible<5->{Funktion \hask{s -> a}} \\

      \visible<2->{\alert<2>{Traversal}} &
      \visible<2->{ein \hask{s} besteht aus \hask{a}'s und anderen Daten} \\

      \alert<1>{Lens} &
      ein \hask{s} besteht aus einem \hask{a} und anderen Daten \\
  ...
  \begin{onlyenv}<8->
    \vspace{-0.7em}
\begin{minted}[fontsize=\footnotesize,baselinestretch=1.2]{haskell}
