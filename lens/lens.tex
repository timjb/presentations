...
step dt = do
  state <- |\hl{use}| gameState
  when (state /= Running) abort
  vy <- velY |\hl{<+=}| dt*gravity
  px <- catPos.x |\hl{<+=}| dt*velX
  py <- catPos.y |\hl{<+=}| dt*vy
  when (py <= -h/2) $ gameState |\hl{.=}| GameOver
  hs <- hurdles |\hl{<\%=}| filter ((> (px-w)) . (^.hurdlePos.x))
  let lastX = fromMaybe (d+w) $
        |\hl{lastOf}| (traverse.hurdlePos.x) hs
  when (lastX < px + 2*w) $ do
    hurdle <- lift $ |\hl{zoom}| gen $ randomHurdle lastX
    hurdles |\hl{.=}| hs ++ [hurdle]
  eps <- |\hl{use}| $ to catExtremePoints
  unless (all id $ passes <$> eps <*> hs) $
    gameState |\hl{.=}| GameOver
\end{haskellcode*}
\end{frame}
}
