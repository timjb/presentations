...
\setbeamertemplate{background}{\includegraphics[width=\paperwidth,height=\paperheight,keepaspectratio]{images/donald-detective-bg.jpg}}
...
\titlegraphic{\includegraphics[width=0.65\linewidth]{images/rubiks-sequence.png}}
\author{Tim Baumann}
...
data Store b a = Store (b -> a) b
  ...
  Dies macht das Paket \nverb{data-lens} von Edward Kmett. (Das ist der Vorgänger seiner \nverb{lens}-Library.)
}
...
\note{
  Diese Einsicht hat Twan van Laarhoven 2009 im Blogartikel \href{http://twanvl.nl/blog/haskell/cps-functional-references}{``CPS based functional references''} veröffentlicht.
}
...
\note{
  Prismen sind dual zu Lenses: Während eine \haskellinline{Lens' s a} den Typ \nverb{s} als Produkt von \nverb{a} und einem zweiten Typ darstellt, gibt ein \haskellinline{Prism' s a} eine Darsellung von \nverb{s} als Summe von \nverb{a} und einem zweiten Typ an.

  Es gibt außerdem noch den Typ \haskellinline{Review t b}. Er entspricht einer Funktion \haskellinline{b -> t}. Unter der Dualität von \haskellinline{Lens'} und \haskellinline{Prism'} entspricht ein \haskellinline{Getter} einem \haskellinline{Review}. Insbesondere ist jedes \haskellinline{Prism' b t} ein \haskellinline{Review t b}.

  \url{https://github.com/ekmett/lens/wiki/Derivation}{Das Lens-Wiki beschreibt} die Herleitung dieser Typen.
}
...
\note{
  Polymorphe Updates wurden zum ersten Mal 2012 in \href{http://r6.ca/blog/20120623T104901Z.html}{einem Blogartikel} von Russell O'Connor beschrieben.
}
...
\note{
  Verwendete Packages: \nverb{lens}, \nverb{text}, \nverb{xml-conduit}, \nverb{wreq}, \nverb{xml-lens}
  Es gibt auch Lenses für JSON-Daten. Diese waren früher in \nverb{lens} enthalten, jetzt sind sie Teil des Pakets \nverb{lens-aeson}.
}
...
\note{
  Im Lens-Wiki auf \lenswithoutdep wird erklärt, wie man Lenses, Traversals, Folds etc. schreiben kann, ohne auf Lens zu dependen.
  ...
  \nverb{Data.Data.Lens} und \nverb{Control.Lens.Plated} basieren auf der \nverb{uniplate}-Library für generisches Programmieren von Neil Mitchell.
...
{\setbeamertemplate{background}{\includegraphics[width=\paperwidth,height=\paperheight,keepaspectratio]{images/donald-detective-wolle-bg.jpg}}
...
\begin{frame}[fragile]
  ...
  \begin{minipage}{0.68 \linewidth}
    \begin{haskellcode*}{fontsize=\footnotesize}
upRows :: Lens' (Cube (Side a)) (Vec4 (Vec3 a))
upRows = horizontalSides . liftVec4 topRow

-- Drehe die oberste Ebene im UZS
...
\note{
  Auch ziemlich cool: \href{http://www.haskellforall.com/2015/01/total-100-exhaustive-pattern-matching.html}{total-1.0.0: Exhaustive pattern matching using traversals, prisms, and lenses}
}
...
main :: IO ()
main = pipeline $ &\hl{traverseOf}\marktikz{h1}& &\hl{template}\marktikz{h2}& hayooLinkModules
  where pipeline :: (Pandoc -> IO Pandoc) -> IO ()
  \end{haskellcode*}
  \begin{visibleenv}<2->
    {\scriptsize
    \begin{tikzpicture}[remember picture, overlay]
      \draw[typeline] (h1) |- + (0, 2.6) node[typebox, anchor=south west] {\verb|:: Applicative f => Traversal s t a b| \\ \verb|-> (a -> f b) -> s -> f t|};
      \draw[typeline] (h2) |- + (0, 0.6) node[typebox, anchor=south west] {\verb|:: forall s a. (Data s, Typeable a)| \\ \verb|=> Traversal' s a|};
    \end{tikzpicture}}
  \end{visibleenv}
\end{frame}
