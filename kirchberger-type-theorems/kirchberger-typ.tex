...
\begin{frame}[plain]
  \begin{theorem}[Kirchberger]
    ...
    Dann sind $P$ und $Q$ genau dann durch eine \alert<2>{Hyperebene} trennbar, wenn für jede Menge $T \subset \E^n$ mit höchstens $n + 2$ Punkten die Mengen $P \cap T$ und $Q \cap T$ durch eine \alert<2>{Hyperebene} trennbar sind.
  \end{theorem}
\end{frame}
...
\begin{frame}
  ...
  \begin{proof}[Beweis von Theorem 9.5]
    ...
    \begin{enumerate}
      ...
      \item<3->{Setze $\Omega := S_1(0) \cap F = \{ x \in F \mid \norm{x} = 1 \}$.}
...
\begin{frame}
  \begin{proof}[Beweis von Theorem 9.5]
    ...
    \begin{enumerate}
      ...
      \item<6-> Somit ist $G := H' \cap F$ ein $(k{-}1)$-dimensionaler Unterraum.
...
% TODO: Ähnlichkeit zu Umformulierung von Carathéodorys Theorem
...
\begin{frame}
  ...
  \begin{theorem}[Carathéodory, Umformulierung]
    Sei $P \subset \E^n$ ($n \geq 2$) nichtleer und kompakt und sei $q \in \E^n$.\\
...
\begin{frame}
  ...
  \begin{proof}<2->
    ...
    \uncover<3->{Es folgt $\{ q \} \cap \conv P \subset \{ q \} \cap B_P = \emptyset$, also $q \not\in \conv P$.\qedhere}
  \end{proof}
\end{frame}
